...
interface QuestionState {
  questions: Query[];
  firebaseQuestions: Record<string, any>;
...
const mergeQuestionsData = (firebaseQuestions: any[]) => {
  ...
  return [...queries].map(query => {
    ...
    if (!firebaseData) {
      console.error(`Query with uid ${query.uid} not found in Firebase data.`);
      return query;
    }
    return {
      ...query,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchQuestions.fulfilled, (state, action) => {
        ...
        state.firebaseQuestions = action.payload.reduce((acc, q) => ({
          ...acc,
          [q.uid]: q
        }), {});
      ...
      .addCase(fetchQuestions.rejected, (state, action) => {
        state.loading.questions = false;
        state.error.questions = action.error.message || 'Failed to fetch questions';
      })
      ...
      .addCase(fetchQuestionData.rejected, (state, action) => {
        state.loading.questionData = false;
        state.error.questionData = action.error.message || 'Failed to fetch question data';
      });
  },
});

export const { setCurrentQuestion, setNormalized, clearErrors } = questionSlice.actions;
