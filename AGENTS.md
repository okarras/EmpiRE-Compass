...
## 5. Dashboard Architecture (Core Areas)

The dashboard is composed of four core areas that work together through shared context, Redux state, and route-level composition in `src/App.tsx`.

### 5.1 Dynamic Questions

- **Purpose**: Enables users to create and execute custom competency questions beyond predefined ones.
- **UI components**: `src/components/DynamicAIQuestion.tsx`, `src/components/AI/DynamicQuestionManager.tsx`, `src/components/AI/SaveDynamicQuestionDialog.tsx`.
- **Backend integration**: Dynamic question APIs are exposed in backend routes (for persistence/management).
- **Data flow**: User input → query/template processing → result rendering in shared question views.

### 5.2 AI Assistant

- **Purpose**: Provides contextual AI analysis over selected question results and chart/data sections.
- **Entry points**: `src/components/AI/FloatingAIAssistant.tsx`, `src/components/AI/AIAssistant.tsx`.
- **State/context**: `src/context/AIAssistantContext.tsx` coordinates assistant visibility, current context, and message flow.
- **Logic hooks**: `src/hooks/useAIAssistant.ts` and `src/hooks/useBackendAIAssistant.ts` manage AI request/response workflows.
- **Architecture note**: Keep provider/context wiring intact; many components call into assistant context (`setContext`, structured prompts, etc.).

### 5.3 Questions (Predefined + Community)

- **Purpose**: Displays canonical question cards, charts, tables, and detailed explanations for RE knowledge exploration.
- **Main components**: `src/components/Question.tsx`, `src/components/QuestionAccordion.tsx`, `src/components/QuestionInformation.tsx`, `src/components/CommunityQuestionAccordion.tsx`.
- **State management**: Question-related data is coordinated through existing Redux slices in `src/store/`.
- **Architecture note**: Prefer extending existing question/display components instead of introducing parallel UI paths.

### 5.4 Authentication

- **Purpose**: Handles Keycloak-based sign-in, guard enforcement, and role-based access for admin capabilities.
- **Core files**: `src/auth/AuthProvider.tsx`, `src/auth/useAuth.ts`, `src/auth/AuthGuard.tsx`, `src/auth/AdminGuard.tsx`, `src/auth/keycloak.ts`.
- **Fallback behavior**: Without Keycloak env vars, the dashboard remains usable in unauthenticated mode while auth/admin-only features are hidden.
- **Architecture note**: Do not remove or bypass auth/admin guards when changing routes/components.

### 5.5 How these areas interact

- `src/App.tsx` composes top-level providers and mounts route/page structure.
- Question screens set context consumed by the AI Assistant for targeted analysis.
- Authentication controls access to admin and protected flows while keeping public research exploration available.
- Backend API calls (including AI proxying) are routed through service layers in `src/services/`.
